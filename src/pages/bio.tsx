...
const ContactPage: React.FC<PageProps> = ({ location }) => {
	...
	const imageSrcs = data.sanitySiteConfig._rawBioImages.map(img => ({ src: img.asset.url }));
	...
	const BioImagesMasonry = (props) => {
		if (imageSrcs.length > 2) {
			...
			return <Masonry items={imageSrcs} render={MasonryImage} columnCount={1} />
		}
	}
	...
	const { breakpoints } = theme;
	return (
		<Container sx={{ pt: [theme.layout.container.paddingTop - 50, theme.layout.container.paddingTop - 20], maxWidth: 'none' }}>
