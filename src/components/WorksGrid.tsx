			...
			{filterTypes.map(a => {
				...
				return (
					<SquiggleLink
						...
						color={active ? 'primary' : 'text'}
						onClick={() => handleClick(a)} key={a}>{a}</SquiggleLink>
				)
			})}
		</Box>
...
export default () => {
	const data = useStaticQuery(graphql`
    {
		...
		allSanityWork(sort: {orderRank: ASC}) {
		  nodes {
			thumbnail {
			  asset {
				gatsbyImageData(aspectRatio: 1.77777777, placeholder: BLURRED, fit: FILLMAX)
			  }
			}
	...
	return (
		<Box sx={{ paddingTop: 100 }}>
			<Box sx={{
				...
				<FlexCenter sx={{
					...
					<FilterButtons
						...
						sx={{ width: '100%', maxWidth: 650, display: 'flex', flexWrap: 'wrap', justifyContent: 'space-evenly' }}
					/>
					{/* <CategoryButtons /> */}
				</FlexCenter>
			</Box>
			...
			<div className="works-grid" ref={gridRef} sx={{
				display: 'grid',
				gridGap: 0,
				gridTemplateColumns: [
